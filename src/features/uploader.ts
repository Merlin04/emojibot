...
const feature1 = async (app: App) => {
    app.message("", async ({ message, say }) => {
		...
		const emojiName = message.text.startsWith(":") && message.text.endsWith(":") ? message.text.slice(1, -1) : message.text;
		...
		const res = await fetch("https://hackclub.slack.com/api/emoji.add", {
			...
			mode: "cors",
			headers: {
				"User-Agent": "Mozilla/5.0 (Windows NT 11.0; WOW64; x64; rv:93.0esr) Gecko/20010101 Firefox/93.0esr/Yp6557blmseFJz",
				"Accept": "*/*",
		...
		say({
			text: res.ok ? `:${emojiName}: has been added, thanks <@${message.user}>!` : `Failed to add emoji:
\`\`\`
${JSON.stringify(res, null, 4)}
\`\`\``,
			thread_ts: message.ts
		});
		if(res.ok) await app.client.reactions.add({
			name: emojiName,
